...
export default function Website() {
  ...
  }, [searchParams, starterWebsite, view, navigate]);
...
`;
  ...
  const openWebsiteManager = async () => {
    if (!currentUser || !brandData || !ideaKey) return;
    ...
    try {
      const { data: existing, error: checkError } = await supabase
        ...
        .select('id')
        .eq('user_id', currentUser.id)
        .eq('brand_name', brandData.selected_name)
        .maybeSingle();

      if (checkError) throw checkError;

      if (existing) {
        navigate(`/manage-website/${existing.id}`);
        return;
      }

      const businessType = businessIdea?.name?.toLowerCase().includes('ecommerce') ||
        businessIdea?.description?.toLowerCase().includes('ecommerce') ||
        businessIdea?.description?.toLowerCase().includes('shop') ||
        businessIdea?.description?.toLowerCase().includes('store')
        ? 'ecommerce'
        : 'service';

      const subdomain = brandData.selected_name.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-');
      ...
      const { data: newWebsite, error: createError } = await supabase
        ...
        .insert({
          ...
          brand_name: brandData.selected_name,
          domain_status: {
            mode: 'subdomain',
            subdomain: subdomain,
            custom_domain: '',
          ...
          primary_color: brandData.brand_colors?.primary || '#2979FF',
          secondary_color: brandData.brand_colors?.secondary || '#06D6A0',
          font_heading: 'Inter',
          font_body: 'Inter',
        })
        .select()
      ...
      await supabase.from('managed_website_content').insert({
        website_id: newWebsite.id,
        home: {
          hero: {
            headline: brandData.selected_tagline || `Welcome to ${brandData.selected_name}`,
            subheadline: businessIdea?.description || '',
          ...
          contact: {
            phone: setupData?.contact_info?.phone || businessPackage.contact.phone || '',
            email: setupData?.contact_info?.email || businessPackage.contact.email || currentUser.email || '',
            city: setupData?.contact_info?.address || businessPackage.location || '',
            service_area: '',
          },
          footer: {
            short_blurb: brandData.selected_tagline || '',
            links: [],
          },
        },
      });

      navigate(`/manage-website/${newWebsite.id}`);
    } catch (err: any) {
      console.error('Error creating managed website:', err);
      alert(err.message || 'Failed to create managed website');
    } finally {
          ...
          {generationError && (
            <div className="mb-6 bg-red-500/10 border border-red-500/30 rounded-xl p-4 flex items-start gap-3">
              ...
              <div className="flex-1">
                <p className="text-red-300 font-medium mb-1">Generation Error</p>
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between flex-wrap gap-4">
                ...
                <div className="flex gap-3">
                  <button
                    onClick={() => navigate(`/marketing-strategy?website=${starterWebsite?.id}`)}
                    className="flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-[#06D6A0] to-[#2979FF] text-white rounded-lg hover:opacity-90 transition-all"
                  >
                    <Megaphone size={18} />
                    Go to Marketing
                  </button>
              ...
              <div className="bg-gradient-to-r from-[#2979FF]/10 to-[#06D6A0]/10 border-2 border-[#2979FF]/40 rounded-xl p-8">
                <div className="text-center">
                  ...
                  <button
                    onClick={openWebsiteManager}
                    disabled={creatingManaged}
                    className="px-8 py-4 bg-gradient-to-r from-[#2979FF] to-[#06D6A0] text-white rounded-lg font-bold text-lg hover:opacity-90 transition-all shadow-lg flex items-center gap-3 mx-auto disabled:opacity-50"
                  >
                    {creatingManaged ? (
                      <>
              ...
              <div className="bg-gradient-to-r from-[#06D6A0]/10 to-[#2979FF]/10 border-2 border-[#06D6A0]/40 rounded-xl p-8">
                <div className="text-center">
                  <Megaphone className="w-12 h-12 text-[#06D6A0] mx-auto mb-4" />
                  <h3 className="text-2xl font-bold text-white mb-3">Market Your Business</h3>
                  <p className="text-gray-300 mb-6 max-w-2xl mx-auto">
                    Generate marketing strategies, social media content, and promotional materials to attract customers.
                  </p>
                  <button
                    onClick={() => navigate(`/marketing-strategy?website=${starterWebsite?.id}`)}
                    className="px-8 py-4 bg-gradient-to-r from-[#06D6A0] to-[#2979FF] text-white rounded-lg font-bold text-lg hover:opacity-90 transition-all shadow-lg flex items-center gap-3 mx-auto"
                  >
                    <Megaphone size={24} />
                    Go to Marketing
                  </button>
